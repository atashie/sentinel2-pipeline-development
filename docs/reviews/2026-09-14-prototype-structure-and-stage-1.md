...
Findings 13 to 15 in [measurements.md](../measurements.md) hold the numbers. In short: whole-object reads beat range reads for the larger sets on this connection. The older copy needs about three times the range requests for its 10 m bands. Its cloud and snow layers are not GeoTIFFs, and its aerosol and water-vapour grids differ. Its reflectance values sit at or below Collection 1's with a dominant difference of exactly 1,000. That is consistent with the provider's offset-and-clamp conversion, while the catalog still declares the offset.
...
4. Network-counter agreement overstated. **Corrected with evidence.** The 46 of 48 claim is withdrawn. Requested bytes and the machine-wide counter stay separately labelled, with the median ratio and the one low reading recorded. The docstring now says machine-wide.
